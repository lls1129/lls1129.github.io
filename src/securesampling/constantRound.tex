...
for product sampling when $\langle \vec{{w}_1},
{\vec{w}_2} \rangle$ is sufficiently large.  Moreover, this
protocol provably leaked no more information than the inner product.
However, this protocol required $O(1/\langle \vec{{w}_1},
{\vec{w}_2} \rangle)$ rounds of communication.
...
Suppose that we are given the value of $\langle \vec{{w}_1},
{\vec{w}_2} \rangle$.  Then, since the expected
number of samples until a collision is a function of $\langle \vec{{w}_1},
{\vec{w}_2} \rangle$, we can just
run the inner loop of protocol $\Pi_{\mathsf{prod}}$ in parallel sufficiently many times
...
is not actually possible to compute $\langle \vec{{w}_1},
{\vec{w}_2} \rangle$ in sublinear communication!
...
that $\langle \vec{{w}_1},
{\vec{w}_2} \rangle = \omega(\log n/n)$. In a similar vein, we assume that the
...
$\cos(\vec{w}_1, \vec{w}_2) = \frac{\langle \vec{w}_1, \vec{w}_2 \rangle}{ \|
...
similarity is usually much larger than $\langle \vec{{w}_1},
{\vec{w}_2} \rangle$. 
...
\BEN
\item[] \hspace{-2em} ${\sf approxIP}(\vec{w}_1, \vec{w}_2)$: 
	~~~ $\rhd$ $\vec{w}_1$ and $\vec{w}_1$ are $n$ dimensional vectors. 
...
slightly abuse the notation and treat the vectors $\vec{w}_1$ and $\vec{w}_2$ as column vectors.)
\EEN

\begin{lemma} {\rm (cf.~\cite[Corollary 3.1]{Kaban15})}
\label{lem:jlt_k}
For all $\vec{w}_1, \vec{w}_2$ such that $\cos(\vec{w}_1, \vec{w}_2)\geq t$, the
procedure ${\sf approxIP}(\vec{w}_1, \vec{w}_2)$ approximates $\langle \vec{w}_1,
...
The proof is found in Section~\ref{apx:pfLemma10}. 
...
\paragraph{Inputs:} Parties $P_1$ and $P_2$ has inputs $\vec{w}_1$ and $\vec{w}_2$
respectively. 
...
\BEN
    ...
    random seed $\llbracket s \rrbracket$. Using this randomness, parties
    homomorphically evaluates $\llbracket PRG(s) \rrbracket$, where $PRG$ is a
    pseudorandom generator, to obtain the JLT matrix $\llbracket \vec M
    \rrbracket$.
    
    \item Each party  $P_b$ homomorphically evaluates $\llbracket \tilde{ \vec{w}}_b \rrbracket =
    \llbracket \vec M \vec{w}_b \rrbracket$.

    \item Party $P_1$ sends $\llbracket \tilde{\vec{w}}_1 \rrbracket$ to $P_2$.  
    
    \item Party $P_2$ homomorphically evaluates $\llbracket \langle \tilde
    {\vec{w}}_1, \tilde{\vec{w}}_2 \rangle \rrbracket$ and sends it to  $P_1$. 

    \item Parties execute threshold decryption to obtain and output
    $\frac 1 k \cdot \langle \tilde{\vec{w}}_1, \tilde {\vec{w}}_2 \rangle$.
\EEN
