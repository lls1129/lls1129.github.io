...
$$D_\ig(\w_1, \w_2) = D_{L_1}(\vec{w'}_1, \vec{w'}_2).$$ 
% \qed
...
Our $L_2$ sampling protocol uses ideal functionality $\F_{L_1}^{ss}$, which works essentially the same as $\F_{L_1}$ except that the output index is secret shared among both parties. We can securely realize this functionality with semi-honest security through a trivial change in the protocol $\Pi_{L_1}$;~for the sake of completeness, we provide the details in Section~\ref{apx:ssL1}.
...
\end{theorem}
%
We provide the proof in Section~\ref{apx:pfTheorem3}.
