...
%     I don't feel strong about saying the Section-result as one of
%     the first results..
%In Section~\ref{apx:otl1}, we give another construction that relies on
...
%hardness proof is in Section~\ref{sec:hahaprooffull}.
%
...
the oblivious sampling is provided in Section~\ref{apx:otl1}.
...
% with communication complexity of just $O(n^{1/2})$ (see Section~\ref{sec:hahaapproxip}).
...
% $\Pi_L^{\Pi'}$ is a sublinear communication protocol that decides $\mathcal{L}$, which is a contradiction to the hardness of $\mathcal{L}$. See Section~\ref{sec:hahathm} and Section~\ref{sec:hahaprooffull} for more details.
