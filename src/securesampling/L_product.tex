...
sampling. This functionality is parametrized by a function $\fl$
...
\item A function $\fl$ describing the leakage.
...
respectively. 
\item Compute $\mathsf{leak}=\fl(\vec{w}_1,\vec{w}_2)$
...
inner product $\ipw$.
(Formally, we set $\fl(\w_1, \w_2) =\ipw$.)  Interestingly, we show that
...
Protocol $\Pp^{IP}$ securely realizes $\Fp$ with leakage $\fl(\w_1,
...
\vec{w}_1, \vec{w}_2 \rangle$,
which is exactly what $\Sim$ does. \qed
