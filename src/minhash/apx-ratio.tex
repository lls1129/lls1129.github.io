...
\end{lemma}
We give the proof later in Section~\ref{sec:tradeoff}.
...
\renewcommand{\negl}{\ensuremath{\mathsf{negl}}} % Force fix right here
\begin{equation*}
\sum_{r \not \in [a+\lg\lg\kappa, b]}  D_r \le k_v \cdot \negl(\kappa) = \negl(\kappa). \tag*{\qedhere}
\end{equation*}
...
&\le \left(  1  + \frac{\zeta \cdot (e^{\zeta \ee/3}+1)}{\etan^{z k_b/n'_R} } \right) \left(E^{n'_R}_r\right)^2  \tag*{\qedhere}
\end{align*} 
\end{proof}
